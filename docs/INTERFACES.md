...
- Alert integration is narrow: `detection_reported(guard_id, position, evidence)` submits observations; `receive_alert_broadcast(position, source_guard_id)` accepts coordinator knowledge. Build 09 alone decides the shared phase and which guards receive broadcasts. `get_radar_snapshot()` returns values only—ID, transform-facing data, state name, suspicion, visibility, last-known position, cone tuning, and alive state—with no target/navigation/state-machine objects.
